...
namespace ConsoleRE
{
	...
	public:
		GFxState(StateType a_state = StateType::State_None) : GRefCountBase<GFxState, 2>(), m_state(a_state) {}
		~GFxState() override = default;
		
		StateType GetStateType() const { return m_state; }
	public:
		StateType m_state;
	};
	static_assert(sizeof(GFxState) == 0x10);
}