...
namespace ConsoleRE
{
	...
	{
		Relocation<Calendar**> Singleton("", 0x20C5250);
		return *Singleton;
	}
	...
	{
		auto     gmst = ConsoleRE::GameSettingCollection::GetSingleton();
	...
	uint32_t Calendar::GetDayOfWeek() const
	{
		return static_cast<uint32_t>(GetDaysPassed()) % 7;
	}
	...
	void Calendar::GetTimeDateString(char* a_dest, uint32_t a_max, bool a_showYear) const
	{
		using func_t = decltype(&Calendar::GetTimeDateString);
		Relocation<func_t> func("", 0x6990C0);
	...
	{
		return gameMonth ? static_cast<uint32_t>(gameMonth->value) : 7;
	}
	...
	{
		return gameYear ? static_cast<uint32_t>(gameYear->value) : 77;
	}
}